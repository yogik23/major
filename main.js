...
async sendBalanceToTelegram(totalBalance) {
    const totalAccounts = this.summary.totalAccounts;
    const message = `⭐️ *Major Report*
    ...
    const chatId = process.env.AUTHORIZED_USERS;
    ...
    try {
        ...
        this.log("Balance sent to Telegram successfully.", "success");
    } catch (error) {
        this.log(`Error sending message to Telegram: ${error.message}`, "error");
    }
}
  ...
  async getTask(token, type) {
    const url = `https://major.glados.app/api/tasks/?is_daily=${type}`;
    try {
  ...
  async doTask(token, taskId) {
    const url = 'https://major.glados.app/api/tasks/';
  ...
  async processDoTask(token) {
    const types = ['true', 'false'];

    for (const type of types) {
        const taskList = await this.getTask(token, type);
        if (taskList) {
            for (const task of taskList) {
                const taskId = task.id;
                const taskName = task.title.replace(/\n/g, '');
                const doTaskStatus = await this.doTask(token, taskId);
                if (doTaskStatus) {
                    this.log(`${taskName}: Completed`, "success");
                } else {
                    this.log(`${taskName}: Incomplete`, "acc");
                }
            }
        }
    }
  ...
  async processAccount(init_data, accountIndex) {
    ...
    try {
      ...
      const tasksresult = await this.processDoTask(access_token);
  ...
  async main() {
    ...
    try {
      while (true) {
        ...
        await this.sendBalanceToTelegram(this.totalBalance);
